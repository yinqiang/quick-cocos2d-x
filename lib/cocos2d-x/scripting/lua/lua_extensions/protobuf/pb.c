...
int iostring_writeInt(lua_State* L) {
	IOString *io = checkiostring(L);
	size_t size = 4;
	int data = lua_tointeger(L, 2);
	char d[4] = {(data & 0xff000000) >> 24, (data & 0xff0000) >> 16, (data & 0xff00) >> 8, (data & 0xff)};
	if(io->size + size > IOSTRING_BUF_LEN){
		luaL_error(L, "Out of range");
	}
	memcpy(io->buf + io->size, d, size);
	io->size += size;
	return 0;
}

int iostring_writeShort(lua_State* L) {
	IOString *io = checkiostring(L);
	size_t size = 2;
	int data = lua_tointeger(L, 2);
	char d[2] = {(data & 0xff00) >> 8, (data & 0xff)};
	if(io->size + size > IOSTRING_BUF_LEN){
		luaL_error(L, "Out of range");
	}
	memcpy(io->buf + io->size, &d, size);
...
static int iostring_readShort(lua_State* L)
{
...
static int iostring_readInt(lua_State* L)
{
    const char* d = lua_tostring(L, 1);
    int offset = 0;
	int ret = 0;
...
static const struct luaL_reg _pb [] = {
    ...
    {"readInt", iostring_readInt},
    {"readShort", iostring_readShort },
    {NULL, NULL}
};
...
static const struct luaL_reg _c_iostring_m [] = {
	...
	{"writeInt", iostring_writeInt}, 
	{"writeShort", iostring_writeShort},
    {"sub", iostring_sub},
